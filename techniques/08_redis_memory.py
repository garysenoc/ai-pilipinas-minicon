"""
Technique 8: Redis-Backed Memory (LCEL Pattern)
===============================================

This technique stores conversation history in Redis, allowing for persistent
storage across sessions and distributed systems. Memory is shared across
multiple instances. Uses modern LCEL pattern.
...
Use Case: Production applications that need persistent memory across
sessions or distributed systems with multiple instances.
"""
...
# Store for Redis chat message histories
store: Dict[str, BaseChatMessageHistory] = {}

def get_session_history(session_id: str) -> BaseChatMessageHistory:
    """Get or create Redis chat message history for a session."""
...
def create_redis_memory_agent():
    """Create an agent with Redis-backed memory using LCEL pattern."""
    ...
    # Wrap with message history (Redis-backed)
...
def demonstrate_redis_memory():
    """Demonstrate Redis memory using LCEL pattern."""
    print("=" * 60)
    print("Technique 8: Redis-Backed Memory (LCEL Pattern)")
    print("=" * 60)
    print("Note: Requires Redis server running")
    print("Using default Redis URL: redis://localhost:6379/0")
    ...
    try:
        chain = create_redis_memory_agent()
